...
def generate_random_username():
    for i in range(100):
        ...
        raise Exception('Called by conftest:random_username after failing to generate a valid username after 100 tries')
    return username
...
def generate_random_email():
    for i in range(100):
        local = ''.join(random.choices(EMAIL_LOCAL_CHARS,k=random.randint(1,64)))
        ...
        raise Exception('Called by random email after failing to generate a valid email after 100 tries')
    return email


def mock_user_list(n=100):
    return [(generate_random_username(),generate_random_password(),generate_random_email()) for _ in range(n)]
