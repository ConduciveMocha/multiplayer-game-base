...
const textValidators = {
    "username": (username) =>  makeLengthValidator(8, 17)(username) ? NO_ERROR : {text:"Username must be between 8 and 16 characters long",field:"username"},
    "password": (password) => makeLengthValidator(1)(password) ? NO_ERROR: {text:'Password cannot be empty',field:'password'}
}
...
const LoginScreen = (props) => {
    ...
    const createHandleOnBlur = field => e => {
        if(field === 'username')  dispatch({type:'UPDATE_USERNAME',value:e.target.value});
    ...
    const createHandleOnChange = field => e => {
        ...
        const usernameError = textValidators["username"](field === 'username' ? e.target.value : state.username);
        const passwordError = textValidators["password"](field === 'password' ? e.target.value : state.password);
        
        // Dispatch actions
        if (usernameError === NO_ERROR && passwordError === NO_ERROR) dispatch({ type: "NOW_VALID" });
        else if (usernameError !== NO_ERROR) dispatch({ type: "NOW_INVALID", error: usernameError });
        else dispatch({ type: "NOW_INVALID", error: passwordError, showError: true });            
    }
...
export default withRouter(connect(
    state=>{return{showComponents:state.ui.showComponents}},
    dispatch => ({
        submitLogin: (username,password) => {dispatch(loginSubmitted(username,password))}
    }))(LoginScreen));